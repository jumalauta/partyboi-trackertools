...
echo "schism routing: .it always rendered with schism (default)"
W="$WORKROOT/caseIT"; mkdir -p "$W"; make_it "$W/tune.it"
run_convert "$W/tune.it"
assert_file "$W/tune.wav" "tune.wav produced"
assert_eq schism "$(backend_used)" "  default: .it routed to schism"

echo "IT_ENGINE=openmpt forces .it onto libopenmpt"
W="$WORKROOT/caseITo"; mkdir -p "$W"; make_it "$W/o.it"
IT_ENGINE=openmpt run_convert "$W/o.it"
assert_eq openmpt "$(backend_used)" "  IT_ENGINE=openmpt: .it -> openmpt"

echo "other PC formats still route to openmpt by default"
W="$WORKROOT/caseXM"; mkdir -p "$W"; make_mod "$W/d.xm"
run_convert "$W/d.xm"
assert_eq openmpt "$(backend_used)" "  default: .xm still routed to openmpt"
