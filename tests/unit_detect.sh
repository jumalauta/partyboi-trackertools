...
echo "detect_backend: .it always -> schism (faithful IT)"
assert_eq schism "$(detect_backend cool.it)"     ".it -> schism by default"
assert_eq schism "$(detect_backend COOL.IT)"     ".it uppercase -> schism"
...
echo "detect_backend: IT_ENGINE overrides"
assert_eq openmpt "$(IT_ENGINE=openmpt detect_backend song.it)"  "IT_ENGINE=openmpt: .it -> libopenmpt"
...
assert_eq openmpt "$(detect_backend foo.xm)"                     "default: .xm stays openmpt"
assert_eq uade    "$(detect_backend foo.mod)"                    "default: .mod stays uade"
